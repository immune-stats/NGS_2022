...
log(cv_ridge_fit$lambda.min)
log(cv_ridge_fit$lambda.1se)

# look at the coefficient values for the chosen lambdas
cv_ridge_coef <- coef(cv_ridge_fit, s = cv_ridge_fit$lambda.1se)
as.data.table(as.matrix(cv_ridge_coef), keep.rownames = "coef")[-1,] %>% 
  ggplot(aes(s1, reorder(coef, s1))) +
  geom_point() +
  geom_vline(xintercept = 0, lty = 2, col = "gray55")

# fitted values
cv_ridge_pred <- predict(object = cv_ridge_fit, s = cv_ridge_fit$lambda.1se, newx = x)
```
...
cv_lasso_fit <- cv.glmnet(x = x, y = y, alpha = 1, standardize = TRUE, family = "gaussian", type.measure = "mse", nfold = 10)
...
# log(cv_lasso_fit$lambda.min)
# log(cv_lasso_fit$lambda.1se)


# look at the coefficient values for the chosen lambdas
cv_lasso_coef <- coef(cv_lasso_fit, s = cv_lasso_fit$lambda.1se)

broom::tidy(cv_lasso_coef)

# fitted values
cv_lasso_pred <- predict(object = cv_lasso_fit, s = cv_lasso_fit$lambda.1se, newx = x)
hist(cv_lasso_pred)
```

```{r}
# calculate the mean-squared error (MSE) for this model
mean((mdna_logodds$Age - cv_lasso_pred)^2)
```
...
cv_fit_a0.5 <- cv.glmnet(x = x, y = y, alpha = 0.5, standardize = TRUE, family = "gaussian", type.measure = "mse", nfold = 10)
plot(cv_fit_a0.5)
```
...
# lambda
cv_fit_elastic_net$lambda.min
cv_fit_elastic_net$lambda.1se


# look at the coefficient values for the lambda
cv_fit_elastic_net_coef <- coef(cv_fit_elastic_net, s = cv_fit_elastic_net$lambda.1se)
broom::tidy(cv_fit_elastic_net_coef)

# fitted values
cv_fit_elastic_net_pred <- predict(cv_fit_elastic_net, s = cv_fit_elastic_net$lambda.1se, newx = x)

hist(cv_fit_elastic_net_pred)
```
